...
npm test
npm run package
cp dist/aitero-assistant-4.0.0.xpi /tmp/aitero-release-first.xpi
npm run package
cmp /tmp/aitero-release-first.xpi dist/aitero-assistant-4.0.0.xpi
cd dist
shasum -a 256 -c aitero-assistant-4.0.0.xpi.sha256
```
...
```sh
unzip -l aitero-assistant-4.0.0.xpi
```
...
git diff --cached
git commit -m "Release AItero Assistant 4.0.0"
...
```sh
git tag -a v4.0.0 -m "AItero Assistant v4.0.0"
...
```sh
gh release view v4.0.0 --repo wgcyeo/AItero
gh release download v4.0.0 \
  --repo wgcyeo/AItero \
  --pattern 'aitero-assistant-4.0.0.xpi*' \
